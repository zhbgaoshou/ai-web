<template>
  <form data-role="login" @submit="login" v-if="tablist[0].active" class="mt-4">
    <label
      class="input input-bordered flex items-center my-4 gap-2 timeline-box"
    >
      <userIcon width="18" />
      <input
        type="text"
    ...
    <div class="flex gap-2">
      ...
      <button type="button" class="btn mt-4" @click="resetForm">
        重新输入
  ...
  <form data-role="login" @submit="register" v-else class="mt-4">
    ...
    <div class="flex gap-2">
      <button class="btn btn-primary mt-4 btn-accent flex-1">
        <span
          v-if="loginLoading"
          class="loading loading-spinner loading-sm"
        ></span
