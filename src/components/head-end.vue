        ...
        <section class="mx-2">
            ...
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="1" class="dropdown-content menu bg-base-200 rounded-box z-[1] w-64 p-2 shadow">
                    <li>
                        <a>
                            <widthIcon width="18" />全屏宽<input type="checkbox" class="toggle"
                                v-model="settingStore.isFullWidth" />
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="1" class="dropdown-content bg-base-300 rounded-box z-[1] w-64 p-2 shadow">
                    <li class="timeline-box hover:-translate-y-1 transition-all my-2 rounded-md cursor-pointer flex justify-between  items-center"
                        :style="{ backgroundColor: value['base-100'] }" v-for="(value, key) in themes"
                        @click="setTheme(key)">

                        <div class="flex gap-2"
                            :style="{ color: value['color-scheme'] === 'dark' ? '#e2e8f0' : '#334155' }">
                        ...
                        <div class="flex justify-center items-center gap-1">
                            <span class="block w-4 h-4 rounded-full" :style="{
                                backgroundColor: value.primary, 'border-radius': value['--rounded-badge']
                            }"></span>
                            <span class="block w-4 h-4 rounded-full"
                                :style="{ backgroundColor: value.secondary, 'border-radius': value['--rounded-box'] }"></span>
                            <span class="block w-4 h-4 rounded-full"
                                :style="{ backgroundColor: value.accent, 'border-radius': value['--rounded-btn'] }"></span>
                            <span class="block w-4 h-4 rounded-full"
                                :style="{ backgroundColor: value.neutral, 'border-radius': value['--tab-radius'] }"></span>
                        </div>
        ...
        <section>
            <div v-if="userStore.isLogin" class="dropdown dropdown-end">
                ...
                <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-40 p-2 shadow">
                    ...
                    <li @click="userStore.logout"><a class="btn">
                            退出登录
                            <logoutIcon width="16" />
                        </a></li>
                </ul>
    ...
    <modal v-model:open="isOpenModal">
        ...
        <div role="tablist" class="tabs tabs-boxed">
            <a role="tab" @click="toggleTab(item)" class="tab" :class="item.active ? 'tab-active' : ''"
                v-for="item in tablist">{{ item.text
                }}</a>
        </div>
...
watch(() => settingStore.isFullWidth, (value: boolean) => {
